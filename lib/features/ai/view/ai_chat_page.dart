...
class _AIChatPageState extends State<AIChatPage> with TickerProviderStateMixin {
  ...
  Color get _greenGlow => _isDarkMode
  ...
  Color get _toolColor {
    return (_tools.firstWhere((t) => t["id"] == _selectedTool)["color"] as Color);
  }
  ...
  void _aplicarFormato(String marcador) {
    ...
    final nuevo = (sel.startsWith(marcador) && sel.endsWith(marcador))
        ? text.replaceRange(selection.start, selection.end, sel.replaceAll(marcador, ""))
        : text.replaceRange(selection.start, selection.end, "$marcador$sel$marcador");
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setM) => Container(
          ...
          child: Column(children: [
            ...
            Padding(padding: const EdgeInsets.symmetric(horizontal: 20),
              child: Row(children: [
                Container(padding: const EdgeInsets.all(8),
                  decoration: BoxDecoration(color: _greenGlow, borderRadius: BorderRadius.circular(10)),
                  child: const Icon(Icons.menu_book_rounded, color: _green, size: 18)),
                const SizedBox(width: 12),
                Text("MIS APUNTES", style: TextStyle(
            ...
            Expanded(child: FutureBuilder<List<Note>>(
              future: _noteService.fetchNotes(Provider.of<AuthProvider>(context, listen: false).token!),
              builder: (context, snap) {
                if (!snap.hasData) return Center(child: CircularProgressIndicator(color: _green, strokeWidth: 2));
                final notas = snap.data!.where((n) {
                  final mT = n.title.toLowerCase().contains(_searchController.text.toLowerCase());
                  final mC = _filtroCategoria == "Todas" || n.categoryName == _filtroCategoria;
                  return mT && mC;
                }).toList();

                if (notas.isEmpty) return Center(child: Text("Sin apuntes", style: TextStyle(color: _textSec)));
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Row(children: [
                          Container(padding: const EdgeInsets.all(8),
                            decoration: BoxDecoration(color: _greenGlow, borderRadius: BorderRadius.circular(10)),
                            child: const Icon(Icons.book_outlined, color: _green, size: 16)),
                          const SizedBox(width: 12),
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Text(nota.title, style: TextStyle(
                              color: _textPrim, fontWeight: FontWeight.w600, fontSize: 14)),
            ...
            if (!_isNoteReadOnly) Container(
              ...
              child: SingleChildScrollView(
                ...
                child: Row(children: [
                  ...
                  Container(height: 24, width: 1, color: const Color(0xFFD4B896), margin: const EdgeInsets.symmetric(horizontal: 6)),
                  DropdownButton<String>(
                    value: _categoriaSeleccionada,
                    underline: const SizedBox(),
                    style: const TextStyle(fontSize: 12, color: Color(0xFF5D4037), fontWeight: FontWeight.w600),
                    items: _categoriasPredeterminadas.map((c) =>
  ...
  void _confirmarEliminar(int id) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text("¿Borrar apunte?", style: TextStyle(color: _textPrim, fontSize: 16, fontWeight: FontWeight.w700)),
        content: Text("Esta acción no se puede deshacer.", style: TextStyle(color: _textSec, fontSize: 13)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx),
            child: Text("Cancelar", style: TextStyle(color: _textSec))),
          TextButton(
            onPressed: () async {
              Navigator.pop(ctx);
              final token = Provider.of<AuthProvider>(context, listen: false).token!;
              if (await _noteService.deleteNote(id, token)) {
                setState(() {}); Navigator.pop(context); _verListadoNotas();
              }
            },
  ...
  void _confirmarEliminarSesion(String sessionId, String token) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: _card,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20), side: BorderSide(color: _border)),
        title: Text("¿Eliminar sesión?", style: TextStyle(color: _textPrim, fontSize: 16, fontWeight: FontWeight.w700)),
        content: Text(
    ...
    return Scaffold(
      ...
      floatingActionButton: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          _fab(Icons.list_alt_rounded, Colors.grey.shade700, _verListadoNotas, "f1", mini: true),
          const SizedBox(height: 8),
      ...
      appBar: AppBar(
        ...
        title: Row(children: [
          Container(
            ...
            decoration: BoxDecoration(color: _greenGlow, borderRadius: BorderRadius.circular(8)),
            child: const Icon(Icons.auto_awesome, color: _green, size: 16)),
          const SizedBox(width: 10),
          const Text("DAIKO AI", style: TextStyle(
        ...
        actions: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: _greenGlow, borderRadius: BorderRadius.circular(20),
      ...
      body: Column(children: [
        Expanded(child: _messages.isEmpty ? _buildEmptyState() : ListView.builder(
    ...
    return Drawer(
      ...
      child: Column(children: [
        Container(
          height: 160,
          decoration: BoxDecoration(
            color: _isDarkMode ? const Color(0xFF001A14) : const Color(0xFFE8F5F0),
            border: Border(bottom: BorderSide(color: _border))),
          child: SafeArea(child: Padding(
            padding: const EdgeInsets.all(20),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(padding: const EdgeInsets.all(10),
                decoration: BoxDecoration(color: _greenGlow, borderRadius: BorderRadius.circular(12)),
                child: const Icon(Icons.auto_awesome, color: _green, size: 22)),
              const SizedBox(height: 12),
              const Text("DAIKO AI", style: TextStyle(
                color: _green, fontSize: 18, fontWeight: FontWeight.w800, letterSpacing: 2)),
              Text("Historial de sesiones", style: TextStyle(color: _textSec, fontSize: 12)),
            ])))),
          ...
          child: GestureDetector(
            ...
            child: Container(
              ...
              child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(Icons.add_comment_outlined, color: _green, size: 16),
                SizedBox(width: 8),
                Text("NUEVO CHAT", style: TextStyle(
                  color: _green, fontWeight: FontWeight.w700, fontSize: 13, letterSpacing: 1)),
              ])))),
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 6),
            Text("RECIENTES", style: TextStyle(
        ...
        Expanded(child: FutureBuilder<List<Map<String, dynamic>>>(
          ...
          builder: (ctx, snap) {
            ...
            final sessions = snap.data!.map((e) => e['session_id']).toSet().toList();

            if (sessions.isEmpty) return Center(
              child: Text("Sin sesiones", style: TextStyle(color: _textSec, fontSize: 13)));
            ...
            return ListView.builder(
              ...
              itemBuilder: (ctx, i) {
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                    color: isActive ? _greenGlow : Colors.transparent,
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(color: isActive ? _green.withOpacity(0.3) : Colors.transparent)),
                  child: ListTile(
                    dense: true,
                    leading: Icon(Icons.chat_bubble_outline_rounded,
                      size: 16, color: isActive ? _green : _textSec),
                    title: Text("Sesión ${sid.substring(0, 6)}",
                      style: TextStyle(
                        fontSize: 13, color: isActive ? _green : _textPrim,
  ...
  Widget _buildEmptyState() {
    return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      Text("DAIKO AI", style: TextStyle(
        color: _textPrim, fontSize: 22, fontWeight: FontWeight.w800, letterSpacing: 2)),
      const SizedBox(height: 8),
      Text("Tu asistente financiero personal", style: TextStyle(color: _textSec, fontSize: 14)),
      ...
      ...[
        ...
        child: GestureDetector(
          onTap: () { _chatController.text = s.$2; _sendMessage(); },
          child: Container(
      ...
      child: Row(
        ...
        children: [
          if (!isUser) ...[
            Container(
              width: 28, height: 28,
              decoration: BoxDecoration(color: _greenGlow, shape: BoxShape.circle),
          ...
          Flexible(
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: isUser ? _green.withOpacity(0.2) : _border, width: 1)),
      ...
      child: Row(children: [
        ...
        GestureDetector(
          ...
          child: Container(
            width: 40, height: 40,
            decoration: BoxDecoration(
              color: _toolColor, shape: BoxShape.circle),
    ...
    return PopupMenuButton<String>(
      ...
      itemBuilder: (ctx) => _tools.map((t) {
        ...
        return PopupMenuItem<String>(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(t["id"] as String, style: TextStyle(
                color: _textPrim, fontWeight: FontWeight.w700, fontSize: 13)),
              Text(t["sub"] as String, style: TextStyle(color: _textSec, fontSize: 11)),
            ])),
      ...
      child: Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(_selectedTool, style: TextStyle(
  ...
  // ── FAB HELPER ────────────────────────────────────
  Widget _fab(IconData icon, Color bg, VoidCallback onTap, String tag, {bool mini = false}) {
  ...
  TextSpan buildTextSpan({required BuildContext context, TextStyle? style, required bool withComposing}) {
    ...
    text.splitMapJoin(regex,
      onMatch: (m) {
        ...
        if (full.startsWith('**')) {
          children.addAll([TextSpan(text: '**', style: fade),
            TextSpan(text: full.substring(2, full.length - 2), style: style.copyWith(fontWeight: FontWeight.bold)),
            TextSpan(text: '**', style: fade)]);
        } else if (full.startsWith('_')) {
          children.addAll([TextSpan(text: '_', style: fade),
            TextSpan(text: full.substring(1, full.length - 1), style: style.copyWith(fontStyle: FontStyle.italic)),
            TextSpan(text: '_', style: fade)]);
        } else if (full.startsWith('~')) {
          children.addAll([TextSpan(text: '~', style: fade),
            TextSpan(text: full.substring(1, full.length - 1), style: style.copyWith(fontSize: 24, fontWeight: FontWeight.bold)),
            TextSpan(text: '~', style: fade)]);
        } else {
